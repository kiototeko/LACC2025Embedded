...
void loop() {
  ...
  if (!fb) {
    Serial.println("⚠️  Frame buffer is NULL");
    delay(100);
    return;
  }

  // Simple threshold example (avoid deref null)
  int brightCount = 0;
  ...
  if (fb->buf) {
    for (int y = h/4; y < 3*h/4; y++) {
      for (int x = w/4; x < 3*w/4; x++) {
        int idx = y * w + x;
        if (idx < fb->len && fb->buf[idx] > 128) {
          brightCount++;
        }
      }
    }
    //digitalWrite(LED_PIN, brightCount > 2000 ? HIGH : LOW);
    Serial.println(brightCount);
    Serial.println(w);
    Serial.println(h);
  } else {
    Serial.println("⚠️  fb->buf is NULL");
  }
